...
def get_hotel_suggestions(
    ...
    Args:
        ...
        sort_by: Sort results by "rating" or "price"
    ...
    try:
        ...
        for hotel in hotels:
            formatted_hotels.append({
                "name": hotel.get("name", ""),
                "address": hotel.get("address") or hotel.get("formatted_address") or "",
...
def _get_mock_hotels(destination: str) -> List[Dict[str, Any]]:
    ...
    mock = [
        {"name": f"{destination} Grand Hotel", "rating": 4.5, "price_per_night": 180.0},
        {"name": f"{destination} City Inn", "rating": 4.0, "price_per_night": 95.0},
        {"name": f"{destination} Luxury Resort", "rating": 4.8, "price_per_night": 280.0},
    ]

    formatted = []
    for hotel in mock:
        name = hotel["name"]
        q_book = quote_plus(f"{name} {destination} book")
        q_map = quote_plus(f"{name} {destination}")
        formatted.append({
            "name": name,
            "address": "",
            "city": destination,
            ...
            "rating": hotel["rating"],
            "bedrooms": 1,
            "url": f"https://www.google.com/search?q={q_book}",
...
def search_hotels_cached(
    ...
    **kwargs
    ...
    """Search for hotels with simple in-memory caching."""
